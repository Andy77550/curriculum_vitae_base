...
if (!empty($id_user)) {
	...
	//Récupérations des expériences professionnelles
	$sql = "SELECT * FROM experience_form WHERE id = $id_user";
	$result = Database::query($sql);
	$xpForm = Database::store_result($result, 'ASSOC');

	//Récupérations des expériences professionnelles
	$sql = "SELECT * FROM experience_hobbies WHERE id = $id_user";
	$result = Database::query($sql);
...
$content = $tpl->fetch('/'.$plugin->get_name().'/view/mini-curriculum.tpl');
// Assign into content
